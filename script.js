$(function () {
    ...
    var GrootsteP;
    var arrayTellerDoelgroep = [];
    var arrayTellerGenre = [];
    ...
    function checkGenres() {
        $.ajax({
            ...
            for (let event of data.items) { //checken door de items, naar de verschillende genres
                ...
                //tellen naar aantal doelgroepen
                 //toevoegen in een nieuwe array
                 //een index met teller en een index met woord om later te kunnen herkennen
                 //waarschijnlijk kan dit samen in de andere if aangezien het zelfde if is
                 //maar 2de is niet hetzelfde dus heb ik het apart gedaan zo leek het meer begrijpbaar 
                if (!arrayTellerDoelgroep.includes(currentDoelgroep)) {
                    arrayTellerDoelgroep.push(1);
                    arrayTellerDoelgroep.push(currentDoelgroep);
                    //console.log("arraytest" + arrayTellerDoelgroep);
                }
                if (arrayTellerDoelgroep.includes(currentDoelgroep)) {
                    var indexc = arrayTellerDoelgroep.indexOf(currentDoelgroep);
                    arrayTellerDoelgroep[indexc - 1] = arrayTellerDoelgroep[indexc - 1] + 1
                    // console.log("arraytest" + arrayTellerDoelgroep);
                }//Tellen naar aantal genres in de data 
                if (!arrayTellerGenre.includes(currentGenre)) {
                    arrayTellerGenre.push(1);
                    arrayTellerGenre.push(currentGenre);
                    // console.log("arraytest" + arrayTellerGenre);
                }
                if (arrayTellerGenre.includes(currentGenre)) {
                    var indexc = arrayTellerGenre.indexOf(currentGenre);
                    arrayTellerGenre[indexc - 1] = arrayTellerGenre[indexc - 1] + 1
                    //console.log("arraytest" + arrayTellerGenre);
                }
            }
            ...
            for (i = 0; i < doelGroepArray.length; i++) { //niet langer dan aantal woorden pushen
                $(".Doelgroep").append(`<button id="btn">${arrayTellerDoelgroep[i*2+1]}(${arrayTellerDoelgroep[i*2]})</button>`);
            } //pushen naargelang positie in de array

            for (i = 0; i < genreArray.length; i++) {
                $(".Genre").append(`<button id="btn">${arrayTellerGenre[i*2+1]}(${arrayTellerGenre[i*2]})</button>`);
            }
        })
    }
    ...
    $('.Doelgroep').on('click', '#btn', function () { //clicken op een button
        ...
        var klikteButton =($(this).text().toUpperCase());
        var indexHaakjes = klikteButton.indexOf("("); //index zoeken van haakje om deze weg te halen
        var geklikteFilter = klikteButton.substring(0, indexHaakjes) //weghalen omdat ik het moet vergelijken in array van data
